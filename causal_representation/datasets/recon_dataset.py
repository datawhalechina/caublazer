...
class ReconDataset:
    def __init__(self,
        ...
        all_variables = list(list(covariates) + list(treat) + list(outcomes) + list(causal_name))
    ...
    def get_origin_x(self):
        return pd.concat((self.data[self.covariates], self.data[self.treat], pd.DataFrame(np.array(self.data[self.treat]) * np.array(self.data[self.causal_name]), columns=['causal_value'])), 1)

    def get_causal_value(self):
        return self.data[self.causal_name]
    ...
    def subset(self, idx, keep_propensity):
        ...
        if keep_propensity and self.propensity is not None:
            ...
            propensity = None
        return ReconDataset(data, self.covariates, self.treat, self.outcomes,
                           propensity)
